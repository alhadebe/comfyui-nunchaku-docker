...
# 1) Wheel (fast) OR source clone (no tmpfs flag here)
...
# 2) Build from source WITHOUT isolation and WITHOUT deps to avoid pulling another PyTorch
WORKDIR ${APP_DIR}/nunchaku
ENV NUNCHAKU_INSTALL_MODE=ALL \
    MAX_JOBS=2
RUN if [ -d "${APP_DIR}/nunchaku" ]; then \
      pip install --no-cache-dir --upgrade pip wheel setuptools ninja && \
